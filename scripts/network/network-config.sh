...
gateway=$(jq '.network | .gateway' $path/../../config/config.json  | tr -d '"')
...
echo "$date [NETWORK][Interface] The network interface $interface has been configured" >> $path/../../logs/network.log
